...
@available(iOS 13, macOS 11, *)
public struct AlertToastCustom: View{
    ...
    /// - `hud`
    /// - `banner`
    public var displayMode: DisplayMode = .banner(.pop)
    
    ///What the alert would show
    ///`complete`, `error`, `systemImage`, `image`, `loading`, `regular`
    public var type: AlertType = .regular
    ...
    ///Full init
    public init(displayMode: DisplayMode = .banner(.pop),
...
public extension View{
    ...
    ///   - alert: () -> AlertToast
    /// - Returns: `AlertToast`
    func showMessage(isPresenting: Binding<Bool>, duration: Double = 3, tapToDismiss: Bool = true, offsetY: CGFloat = 0, alert: @escaping () -> AlertToastCustom, onTap: (() -> ())? = nil, completion: (() -> ())? = nil) -> some View{
